...
- **FastAPI** - High-performance async API layer serving as the primary web framework
- **Python 3.10+** - Core implementation with asynchronous, thread-safe architecture

### AI/ML Frameworks

- **FAISS (Facebook AI Similarity Search)** - Vector database for persistent incident memory
- **SentenceTransformers** - Embedding framework using MiniLM models
- **Hugging Face Transformers** - ML model orchestration
- **Hugging Face Inference Router** - AI inference management

### UI Framework

- **Gradio** - Interactive observability dashboard (served at localhost:7860)

### Data Validation

- **Pydantic v2** - Type-safe data modeling with frozen models for immutability
...
The framework essentially combines **FastAPI** for the API layer, **FAISS** for vector memory, **Gradio** for observability, all orchestrated through a custom multi-agent Python architecture designed for production reliability.
